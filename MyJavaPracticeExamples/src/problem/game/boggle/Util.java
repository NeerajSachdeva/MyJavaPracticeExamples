...
public class Util {
	 ...
	 *
	 * @param size
	 *            the size of puzzle. The maximum lenght of words in the returned trie is (size * size)
	...
	public static Trie getTrie(int size) {
		...
		final File file = new File("D:/Personal_Project/my_personal_workspace/Words.txt");
		try {
		...
		// System.out.println("Finish reading the dictionary");
		return trie;
	}
	...
	/**
	 * Returns the matrix of connectivity between two points. Point i can go to point j iff matrix[i][j] is true Square (x, y) is equivalent
	 * to point (size * y + x). For example, square (1,1) is point 5 in a puzzle of size 4
	 *
	 * @param size
